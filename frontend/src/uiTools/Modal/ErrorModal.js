...
const ErrorModal = ({ errormsg, setErrormsg }) => {
    const handleErrorMessage = (msg) => {
        if (msg.includes("missing revert")) {
            return "Please reload this page";
        }
        if (msg.includes("https://")) {
            return "";
        }
        if (msg.includes("invalid address") || msg.includes("unknown account")) {
            return "Please open MetaMask and login";
        }
        if (msg.includes("invalid contract address or ENS name")) {
            return "Invalid contract address or ENS name provided.";
        }
        return msg;
    };

    const message = handleErrorMessage(errormsg?.msg || "");
                ...
                <div className="relative p-4 w-full max-w-md h-full md:h-auto z-50">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <button
                            ...
                            onClick={() => setErrormsg({ status: false, msg: "" })}
                        >
                            <svg
                                aria-hidden="true"
                                className="w-5 h-5"
                        ...
                        <div className="p-6 text-center overflow-hidden">
                            ...
                            <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                {message ? getOnlyErrorText(message) : "No Error"}
                            </h3>
                            <button
                                onClick={() => setErrormsg({ status: false, msg: "" })}
                                data-modal-toggle="popup-modal"
